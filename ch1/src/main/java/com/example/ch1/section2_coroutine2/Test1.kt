...
// scope 를 만들어 코루틴을 구조화
// custom scope 선언
// CoroutineScope 인터페이스를 구현한 클래스의 객체가 코루틴 스코프
class MyScope: CoroutineScope {
    val scopeJob: Job = Job()

    // 이 scope 안에서 동작하는 모든 코루틴을 위한 공통 정보 설정
    // Dispatchers.Default: 이 스코프의 코루틴들은 Dispatchers.Default 스레드 풀에서 동작
    // scopeJob: 스코프 잡을 선언 이렇게 되면 코루틴들의 잡이 이 job 의 하위 job 이 됨
